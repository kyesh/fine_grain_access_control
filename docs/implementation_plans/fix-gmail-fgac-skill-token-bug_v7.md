...
**Test 5: QA Acceptance Test 09 — Universe Domain Rollback**
Run all 8 test cases from `docs/QA_Acceptance_Test/09_universe_domain_rollback.md`:
1. Landing page copy (browser — `goto` + `snapshot`)
2. Setup page credentials flow (browser — `goto` + `snapshot`)
3. Dashboard key creation flow (browser — `goto` + `click` + `snapshot`)
4. API endpoint integration (curl)
5. User guide documentation (grep)
6. ADR existence (file check + grep)
7. gmail-fgac skill pattern (grep)
8. Source code sweep for remaining `universe_domain` references (grep)

**Test 6: QA Acceptance Test 10 — OpenClaw gmail-fgac Skill ⭐**
Run the quick regression checklist from `docs/QA_Acceptance_Test/10_openclaw_gmail_fgac_skill.md`:
```bash
...
```
Then run all 11 test cases for full coverage if the smoke test passes.
...
A formal QA test to verify that all universe_domain artifacts have been removed from the public UI and that the `api_endpoint` auth pattern works end-to-end. 8 test cases covering UI copy, documentation, and source code sweeps.

#### [NEW] `docs/QA_Acceptance_Test/10_openclaw_gmail_fgac_skill.md` ⭐ Primary Regression Test

The **primary repeatable regression test** for the gmail-fgac skill. 11 test cases covering:
- Skill installation & dependency validation
- All 6 actions: `labels`, `list`, `read`, `send`, `forward`, `attachment`
- Security enforcement: send whitelist blocks, read blacklist blocks
- Error handling: invalid accounts, missing arguments
- JSON output contract stability for agent parsing
- **Quick regression checklist**: 4-command smoke test for rapid re-runs after any skill change

> This test should be run after every modification to the skill code, proxy backend, or access control schema.
