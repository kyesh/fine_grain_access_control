...
### ~~2E: CLI OAuth~~ — ELIMINATED
The standalone `npx fgac auth login` npm package has been **cut from scope**. CLI users use the same local scripts shipped with the OpenClaw skill and Claude Code CLI plugin. No separate npm package needed — the scripts handle OAuth, token storage, and refresh directly.
...
- Option B: CLI Mode (local scripts — not yet implemented, see 3D)

> [!WARNING]
> **SKILL.md Option B references scripts that don't exist yet.** The current Python example uses `FGAC_PROXY_KEY` env var (API key mode), not the planned OAuth-in-scripts flow. Needs update when shared scripts are built (Phase 3D).
...
- SKILL.md guidance: scripts mirror the Gmail API but at `gmail.fgac.ai` instead of `googleapis.com`

### ~~3C: CLI Distribution~~ — ELIMINATED
No separate npm package. CLI users use the same local scripts as OpenClaw and CC CLI.

### 3D: Shared Scripts + CC CLI Plugin ❌ NOT STARTED
The OpenClaw scripts (once rewritten for FGAC OAuth in 3B) are **shared** across:
- **OpenClaw** — installed to `~/.openclaw/skills/gmail-fgac/`
- **Claude Code CLI** — referenced from SKILL.md Option B

**What ships:** `auth.js`, `gmail.js`, `accounts.js`, `shared.js` — all with FGAC OAuth built in. Both SKILL.md files give guidance that the scripts mirror the standard Gmail API but route through `gmail.fgac.ai`.

### 3E: Documentation ✅ COMPLETE
...
- [distribution_architecture.md](file:///home/kyesh/GitRepos/fine_grain_access_control/docs/distribution_architecture.md) — canonical reference for 4-package model
...
| 2 | Package shared scripts for CC CLI (same code as #1) | 3D | Step 1 | 🔴 Critical |
