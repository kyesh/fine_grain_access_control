...
## Phase 0: Clerk Spike ✅ COMPLETE

> [!NOTE]
> **Spike completed 2026-05-09.** All 5 capabilities validated. Two adjustments needed:
> 1. Clerk's consent page is built-in — can't be replaced with custom agent profile picker
> 2. OAuth tokens only contain `userId` — no custom claims injection
> 
> **Solution:** Post-auth profile selection via `select_profile` MCP tool. See [full results](file:///home/kyesh/GitRepos/fine_grain_access_control/docs/spike_results/clerk-mcp-oauth.md).
...
### Agent-Scoped OAuth Flow (Revised per Spike Results)

```
  Agent → MCP endpoint → 401 with resource_metadata URL
    → Agent discovers Clerk via /.well-known/ endpoints
    → Agent does DCR (auto-registers, gets client_id)
    → Agent initiates Auth Code + PKCE
    → User signs in via Clerk (Google sign-in)
    → Clerk's built-in consent page (can't customize)
    → Token issued with userId only (sub claim)
    → Agent connects to MCP, calls select_profile first
    → MCP server resolves userId → user → lists agent profiles
    → If 1 profile: auto-selected
    → If 0 profiles: auto-created with defaults
    → If N profiles: returns list, agent picks one
    → All subsequent tools enforce selected profile's permissions
```
...
|------|------|-------------|
| `select_profile` | FIRST call | Lists/auto-selects agent profile. Required before any Gmail tool. |
| `gmail_list` | After profile | List emails (search, filters) |
| `gmail_read` | After profile | Read full message |
| `gmail_send` | After profile | Send email (subject to rules) |
| `gmail_forward` | After profile | Forward email (subject to rules) |
| `gmail_labels` | After profile | List labels |
| `gmail_attachment` | After profile | Download attachment |
| `get_my_permissions` | After profile | Show this profile's rules, email grants, blocked patterns |
| `request_permission` | After profile | Request additional access (pending user approval) |
...
- Add `mcp_sessions` table (stores profile selection per session)
- Keep `proxyKeys` as a view/alias during migration for backward compat

### New Routes
- `src/app/mcp/route.ts` — MCP handler (promoted from spike)
- `src/app/.well-known/*` — OAuth discovery (already built in spike)
- `src/lib/mcp-tools.ts` — Shared tool implementations
- Remove `src/app/oauth/consent/page.tsx` — NOT NEEDED (Clerk's built-in consent suffices)
- Remove `src/app/oauth/callback/route.ts` — spike-only, not needed for production MCP
