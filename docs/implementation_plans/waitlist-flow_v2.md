...
### Landing Page CTAs
- **`src/app/layout.tsx`**:
  - In the top-right navbar (`<Show when="signed-out">`), replace the `<SignInButton mode="modal">` with `<SignUpButton mode="modal">` and change the button text to "Sign Up". This routes new users to the Clerk Waitlist process, while existing users can click the "Sign in" option at the bottom of the modal.
- **`src/app/page.tsx`**:
  - In the main body section (`<Show when="signed-out">`), completely remove the "Sign In" button so that only the "Join Beta Waitlist" CTA (which goes to the custom `/waitlist` page) is available.
