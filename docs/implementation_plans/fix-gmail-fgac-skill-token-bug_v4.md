...
2. Grep for remaining `universe_domain` references outside of `docs/adr/` and `docs/implementation_plans/` — should be zero in public-facing files

### Manual Verification
1. Visit setup page — confirm no "zero code changes" claim
2. Visit landing page — confirm updated copy
3. Create an API key on dashboard — confirm primary flow shows Copy key + endpoint
4. Run the gmail-fgac skill with the `api_endpoint` pattern against production proxy
5. Deploy to preview via `/deploy-pr-preview` and run through applicable QA tests
