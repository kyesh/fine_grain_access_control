...
└──────────────────────────────┬──────────────────────────────────┘
         ...
         ▼                     ▼                     ▼
┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
│  Hosted MCP     │  │  Claude Code    │  │  OpenClaw       │
│  (curl)         │  │  (tmux)         │  │  (Docker)       │
│                 │  │                 │  │                 │
│  ✓ send wl      │  │  ✓ send wl      │  │  ✓ send wl      │
│  ✓ read bl      │  │  ✓ read bl      │  │  ✓ read bl      │
│  ✓ multi-email  │  │  ✓ multi-email  │  │  ✓ multi-email  │
│  ✓ connections  │  │  ✓ connections  │  │  ✓ connections  │
│  ✓ key lifecycle│  │  ✓ key lifecycle│  │  ✓ key lifecycle│
...
└─────────────────┘  └─────────────────┘  └─────────────────┘
         ...
         └─────────────────────┼─────────────────────┘
              ...
              │  plugin marketplace /          │
              │  claude mcp add               │
              │  → Run ALL caps against prod  │
              └────────────────────────────────┘
```

**Every agent interface tests every capability.** Capability docs are shared assertion checklists. Agent interface docs describe *how* to execute those assertions in that specific runtime.
...
│
├── agents/                                # LAYER 2: Per-agent test runbooks
...
│
├── production/                            # LAYER 3: Install from real channels + verify
│   ├── 01_smoke_test.md                   # Discovery endpoints, 401, basic auth
│   └── 02_distribution_channels.md        # ClawHub install, plugin marketplace, claude mcp add
│
...
4. Execute setup docs IN ORDER using /browser-agent:
...
This doc runs ALL capabilities from `capabilities/` via curl against the MCP endpoint.
```
...
```markdown
---
description: Validate production via real distribution channels
---
...
1. `docs/QA_Acceptance_Test/production/01_smoke_test.md`
2. `docs/QA_Acceptance_Test/production/02_distribution_channels.md`

Distribution channels doc installs from ClawHub, plugin marketplace,
and claude mcp add, then runs ALL capabilities against production URLs.
```
...
Tests the **actual user experience** of installing FGAC from real channels:

| Channel | Install Method | Expected URL |
|---------|---------------|-------------|
| ClawHub | `clawhub skill install gmail-fgac` | `gmail.fgac.ai` (built into SKILL.md) |
| Claude Code Marketplace | `/plugin marketplace browse` → search "fgac" | `fgac.ai/api/mcp` |
| Claude Code Manual | `claude mcp add --transport http fgac-gmail https://fgac.ai/api/mcp` | `fgac.ai/api/mcp` |
| CLI Scripts | Copy from SKILL.md Option B instructions | `gmail.fgac.ai` |

After install, run ALL capabilities against production URLs (no `FGAC_ROOT_URL` override).
...
| **NEW** | `.agent/workflows/qa-production.md` | Pointer → `production/` docs |
...
| **NEW** | `docs/QA_Acceptance_Test/production/` (2 files) | Real distribution channel install + verify |
...
### 1Password
- `op vault list` shows vaults
- `bash scripts/qa-secrets.sh` populates `.qa_test_emails.json`

### Per-Agent Verification
| Workflow | What it validates |
|----------|------------------|
| `/qa-setup` | Dashboard screenshot shows keys + rules + both emails |
| `/qa-hosted-mcp` | All capabilities pass via curl against MCP endpoint |
| `/qa-claude-code` | All capabilities pass via tmux Claude Code session |
| `/qa-openclaw` | All capabilities pass via genuine OpenClaw Docker gateway |
| `/qa-production` | Install from real channels + all capabilities against `fgac.ai` |
