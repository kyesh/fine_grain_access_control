# Agent QA: Comprehensive Environment & Distribution Plan (v6)
...
**Key Change for Local Testing**: We will NOT copy files (`cp -r`). Copying introduces drift. Instead, we use native tooling to load the live `docs/skills/` code directly:
- **Claude Code CLI** uses the `--plugin-dir <path>` flag on launch.
- **OpenClaw** uses a direct Docker volume mount to the local repository path.
...
│   └── state.json                  # Runtime token storage (not /tmp)
│
...
└── openclaw/                       # Workspace for OpenClaw Docker (port 18790)
    ...
    ├── docker-compose.yml          # Configured to mount local repo
...
|-------|------|---------------------|------------|---------------------|
| **Local** (`agents/`) | Validate new skill features, rules, and access scoping. | Native local loading (`--plugin-dir`, volume mounts, `claude mcp add localhost`). | `localhost:3000` | Full Suite (List, Read, Send, Block, etc.) |
...
services:
  testclaw:
    ...
    volumes:
      ...
      # Directly mount the live repo skill directory
      - $REPO_ROOT/docs/skills/gmail-fgac:/home/node/.openclaw/skills/gmail-fgac
