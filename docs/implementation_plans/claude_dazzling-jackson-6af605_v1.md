# Approve page: the "opened, never approved" leak, re-read per person (v1)

Branch: `claude/dazzling-jackson-6af605` · Date: 2026-09-09 · Builds on PR #123
...
- Unit: `mcp:lint` scripts pass; `tsc --noEmit` and eslint clean on the changed files.
- Local (dev server + branch DB, `qa-env-runner`, QA account): capability 17 A2, A12,
  A13 and the `picker_token_requested` half of A14 / 16 A21 — see v2 for results.
- Preview (`/deploy-pr-preview`): same scope — see v2.
