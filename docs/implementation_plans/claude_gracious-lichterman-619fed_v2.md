...
Branch: `claude/gracious-lichterman-619fed` · 2026-09-04 · v2 (after the first preview validation) · from the 2026-09-04 analytics review
...
## v2 — what the first preview validation added

The PR #117 preview (commit 0feae42) validated as designed for the own
account (`google_token: 'ok'`, healthy `gmail_list`, old text gone, bogus
sheet id → 🚫 rule denial). The delegated QA mailbox, however, failed on every
call with `google_token_failure: 'clerk_error'` and the ❌ retry-first text —
consistently, retry included. Preview runtime logs showed the Clerk error:
`Not Found`. A read-only probe of the dev Clerk instance from `.env.local`
returned tokens for BOTH QA accounts in ~100–200 ms, and a probe with a
made-up user id returned the same shape: `ClerkAPIResponseError` status 404,
code `resource_not_found`.

Cause: the preview database is a copy of production, so the delegator's
`users.clerk_user_id` is a production-instance id the dev Clerk instance has
never seen (the "Clerk instance must match the database" rule in CLAUDE.md).
Environment artifact, not a regression — but it is a real deterministic class
the v1 classifier lumped in with transient errors: a delegator whose FGAC
account was deleted (the tombstoned-orphan population) 404s on every call, and
neither retrying nor a reconnect link helps.

Changes in v2:

- `classifyClerkTokenError`: Clerk 404 / `resource_not_found` →
  `owner_not_found`, not retryable, deterministic → 🚫 refusal with
  `denial_code: 'google_token_unavailable'`, whose text says the owner's FGAC
  account is missing, that a reconnect link cannot repair it, and that the
  owner must sign in to FGAC again (then re-delegate, for a delegated mailbox).
  No reconnect link in that text.
- New `reconnectRepairs(reason)` (`no_token` / `refresh_failed` only) — what
  `list_accounts` mints a reconnect link for; `owner_not_found` reports
  `google_token: 'unavailable'` + the failure class with no link.
- Tests for the 404 shape and the new text; docs note that on a preview every
  delegated mailbox is expected to land in `owner_not_found`.

Expected on the re-validated preview: the delegated `gmail_list` returns the
🚫 `owner_not_found` text (environment artifact, now labelled honestly) and
`list_accounts` shows `google_token_failure: 'owner_not_found'` with no
`reconnect_url` for that entry.
