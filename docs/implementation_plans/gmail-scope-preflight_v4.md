# Gmail scope pre-flight check + missing-scope observability (v4)
...
## v4 revision (2026-08-28): one-click reconnect link + proxy-route parity

Ken asked for the remediation link to take the user straight to Google's
consent screen. A direct Google OAuth URL cannot be pre-minted (Clerk builds
it client-side with a per-session state), so the closest achievable is an
auto-fire landing page:

- `?reconnect=1` on `/dashboard/accounts` auto-runs the shared reconnect leg
  once Clerk hydrates, redirecting straight to Google's consent screen — one
  click from the agent's error message to Google. Auto-fire only takes the
  SAFE branch (external account `verified` → `reauthorize` with
  `oidcPrompt: 'consent'`, already forced by `startGoogleReconnect`); the
  destroy+recreate branch stays behind the explicit button click.
- The accounts-page reconnect now explicitly requests `gmail.modify` alongside
  `drive.file` — only listing the scope guarantees Google re-presents the
  unchecked Gmail checkbox to the locked-out population.
- Every reconnect pointer in MCP tool errors (gmail scope denial, Google 401,
  403 scope-reason branch, 403 hedged default, token-unavailable) now links
  `?reconnect=1`. PostHog `google_reconnect_started` gains `source:
  'agent_link'` to measure the funnel.
- The legacy REST proxy route got the same Gmail-scope pre-flight (403 JSON
  with the one-click link, `google_scope_missing` with `via: 'proxy'`),
  closing the gap previously deferred to a separate task.
