...
**Date:** 2026-08-16
**Status:** v2 — adds deploy-time adoption of legacy unflagged default keys

## Revision 2 (2026-08-16, post-preview QA)

v1 left a coverage gap discovered on the preview: pre-instant-start signups
created keys labeled `Default Profile` WITHOUT `is_default`, and the runtime
adoption (`ensureDefaultProfile`) only fires when a NEW MCP connection
resolves. A user whose agents are already connected never triggers it — so
migration 0008's backfill skipped their key, `createDelegation`'s sync
no-ops for them, and the delegated mailbox never appears despite an active
delegation. Observed concretely: `kenyesh2@gmail.com` on the preview DB, with
an active delegation from `kyesh@umich.edu`, showed only its own mailbox.

Fix: **migration 0009** (`0009_adopt_legacy_default_profiles.sql`) — a SQL
mirror of the adoption branch, run every deploy (idempotent):
1. Flag the oldest live `Default Profile`-labeled key for every user row that
   has no flagged default yet.
2. Re-run the 0008 delegated-access backfill so newly adopted keys pick up
   active delegations in the same deploy (0008 executes before 0009).

Verified on the isolated Neon branch: zero unflagged default-labeled keys
remain, zero active delegations unmaterialized on live default keys.

Also noted during the same QA pass: revoking a delegation deletes
delegation-backed rows from custom profiles too (by design), and re-granting
restores only default profiles — custom profiles remain re-opt-in.
