...
| **Publish** | 10 | Commit & push — marketplace is live as soon as the branch merges |
| | 11 | Test marketplace install: `/plugin marketplace add kyesh/fine_grain_access_control` → install works |
| **Preview** | 12 | Deploy PR to preview via `/deploy-pr-preview` |
| | 13 | Verify marketplace install works with preview API endpoints |
| **Production** | 14 | Update `production/03_claude_code_cli.md` |
| | 15 | Validate production install from marketplace → capabilities against `gmail.fgac.ai` |
...
7. `/plugin marketplace add kyesh/fine_grain_access_control` → install succeeds
