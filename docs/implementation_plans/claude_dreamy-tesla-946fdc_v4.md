# Approve-page double submit: pending guard + replay-safe analytics (v4)
...
### Preview (PR #122, deployment of 147d2c7, 2026-09-05 19:33–19:37 UTC)

Preview URL: the `fine-grain-access-control-7k1b4hhow-…vercel.app` deployment
(GitHub commit status ties it to 147d2c7). Fresh Neon preview branch, so no
cleanup was needed; the second scenario used a `sheets_write` link on the same
sheet (a different grant level is "fresh" after read-only was granted). QA
runner, built-in browser, USER_A; `environment='preview'` PostHog events
re-queried from the main session:

| part | client | POSTs | rules | approved | replayed | Google verify |
| --- | --- | --- | --- | --- | --- | --- |
| B — sheets_expose, 6 clicks | click 0 enabled, clicks 1–5 `disabled` "Approving…" | 1 | 1 (Read Only) | 1 | 0 | 1 |
| C — sheets_write, `requestSubmit()` ×3 | success page | 3 | 1 (Read & Write) | 1 | 2 (`picked`) | 1 |
| D — send_whitelist, `requestSubmit()` ×2 | success page | 2 | 1 | 1 | 1 (`grant_active`) | n/a |

Re-opening the granted sheets link renders "✓ Already approved" with no form
and emits only `approval_link_opened`. No console errors. Capability 14 A14 /
A15 and capability 16 A20 pass; the rest of both suites were out of scope.

First preview build of this branch failed: React's production bundle has no
`act()`, and `next build` runs `mcp:lint` with `NODE_ENV=production`. The test
now forces the development build before React loads.

### Numbers the daily review should use from here

`approval_link_approved` = grants written; `approval_link_replayed` = duplicate
submits; conversion = `uniq(request_id)` approved / `uniq(request_id)` minted
(`docs/monitoring.md` 7.14). Raw approve counts before the production deploy of
this PR stay inflated and are not comparable.
