# Docs approval links: where the 0-of-13 loss actually is (v4)
...
## Preview, merged build (PR #126 @ c1088ba = main d25ae3f with PR #123 and #125, `…-o6z0avpo7-…vercel.app`, 2026-09-09 12:30–12:42 UTC)

Regression pass of the same scope after merging main; `mcp:lint` and `tsc`
clean locally. Path B for every step; no sign-in wall.

- **USER_A.** Docs and sheets Picker `nav` unchanged (two entries); flat view
  selected on open with the fixtures at the root; "Shared drives" tab reads
  "No documents." / "No spreadsheets.". Docs approval end to end on the new
  origin, including PR #123's behaviour now on main: the denial carries rule
  (4), the pick-first panel names the Google id with the by-name hint, a
  Picker cancel renders `docs-flow-pick-cancelled` with "Try again — open the
  picker", and Try again re-opens the Picker with the new doc as the first
  root tile → verified confirm → 303 → `approved-verified` (post_approval →
  `ok`) → `docs_read_document` returns the document → `doc_read` rule listed.
- **USER_B.** "Documents" tab (this time listing the account's own Workspace
  files — Drive-side variance, same `nav` served) + "Shared drives" with both
  drives; inside one: 10 folders + a Google Doc (docs) / 4 spreadsheets
  (sheets). Zero FGAC console errors anywhere.

Ready to deploy next: `monitoring.md` keeps main's 7.16–7.18 (PR #125) and
this branch's query as 7.19.
