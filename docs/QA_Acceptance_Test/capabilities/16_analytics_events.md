...
### A15: rule_saved carries via and file_id for a manual rule creation
- On `/dashboard`, create a sheets (or docs) rule by hand-typing a known
  fixture file id into the manual rule form (the same action capability 17
  A10 exercises — one creation serves both). Then query
  `rule_saved` events for the run window
  (`npx tsx scripts/qa-posthog-events.ts --event rule_saved --since <run
  window> --environment <tier>`, or the PostHog MCP connector).
- **Expected**: ≥ 1 row with `via = 'dashboard_manual'`, `mode = 'create'`,
  the correct `service`/`action_type`, and `file_id` equal to the typed id.
  No row carries a `regexPattern`/pattern text property — only
  `pattern_kind`/`pattern_length` shape props ever leave the server. (The
