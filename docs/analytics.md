...
| `approval_link_notified` | server (`src/lib/approvalNotify.ts`, from every mint path) | `channel` (`email`), `trigger` (`repeat_mint`), `action`, `request_id`, `link_count` (2 for a send denial: the per-recipient and send-to-anyone links ride in one email), `mint_count` (how many times the agent had asked), `hours_since_first_mint`. Fires at most once per request: when an agent asks for the SAME link again at least 5 minutes after the first ask and the approve page has never been opened for it, FGAC emails the link from its own support mailbox (`SUPPORT_SMTP_USER`, SMTP app password — never a user's Google grant; that design was rejected 2026-09-15). Capped at 3 per person per rolling 24 h, enforced inside the atomic claim on `approval_requests.notified_at`. Why: ~half of minted requests were never opened (14 d to 2026-09-13, per request), concentrated on agent surfaces that collapse the tool result. Simulated on 30 d of production mints: 42 emails to 29 people, max 2 to one person in a day. Not a funnel stage: join to `approval_link_minted` on `request_id`; compare opens by `link_source`. Runbook: `monitoring.md` 7.23 |
...
crawlers with `client_class` (since 2026-09-10: the MCP Registry listing
brought ~60 distinct crawlers and health probes in its first nine hours, and
`direct_client_fingerprints` went 11 → 96 on scanner traffic alone; many of
them run on a stock `node`/`undici`/`python-httpx` user-agent and are only
identifiable by `client_name`, which is why the class reads both fields —
`monitoring.md` 7.21). Named queries: `monitoring.md` 7.5 (attempts),
