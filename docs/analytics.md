...
| `google_scope_missing` | server (MCP `getGoogleToken`) | `via` (`mcp`), `account_delegated`. Fires when Clerk returns a live Google token whose granted scopes lack Gmail (`gmail.modify` / `mail.google.com`) — the "Gmail checkbox left unchecked at consent" state, which 403s every Gmail call until reconnect. Unsampled and independent of `$mcp_tool_call`, so `uniq(person)` is the size of the locked-out population; the same call carries `google_scope_missing: true` on `$mcp_tool_call`, and Gmail tools then pre-flight-deny with `failure_reason: 'gmail_scope_missing'` (outcome `failed`) instead of surfacing Google's 403 (outcome `error`). Added 2026-08-28 after repeated per-user gmail_list 403s |
