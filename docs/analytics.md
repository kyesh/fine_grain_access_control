...
| `flyer_scanned` | server (`/go/[slug]` short-link redirect — QR flyers AND the growth-prospecting reply links) | `slug`, `slug_known`, `campaign`, `variant`, `channel`, `destination`, `device`, `$raw_user_agent`/`$useragent` (the visitor's UA — without it PostHog's virtual traffic classification marks every scan `$virt_is_bot` / `no_user_agent` and bot-filtered views drop all real scans; added 2026-08-30), `geo_city`/`geo_country` (from Vercel headers; `$geoip_disable` set so the server IP isn't resolved). **Bot-filtered** (link-preview crawlers get the redirect but no event) and captured with a random distinct id per scan — `count()` is scans, not people. Joins to the web funnel via `utm_content`/`ref` = slug on the landing `$pageview`. The appended `utm_source`/`utm_medium` are `qr`/`flyer` for every campaign except `prospecting`, whose rows attribute to their `channel` column (`hn`/`reddit`/`github`/`x`) with `utm_medium=reply` — `src/lib/shortLinkUtm.ts`; see docs/growth-prospecting.md. The event name is historical: for prospecting slugs a "scan" is a click on a link Ken pasted into a manual reply. Links managed via `npm run links` (`scripts/short-links.ts`); per-slug running totals also live in the `short_links.scan_count` column |
