...
| `approval_link_minted` | server (`/api/mcp` — policy denial, send denial, `request_access`) | `action`, `request_id`, `target_hash`, `mint_count`, `via` (`send_denial`/`request_access`; absent for policy denials), and on `request_access` mints since 2026-09-08 `has_resource_name` (the agent passed the file's title, stored in `approval_requests.resource_name` and shown on the approve page — the only name source for a file Google does not share with FGAC yet). **Fires once per mint ATTEMPT**, so `uniq(request_id)` is demand and `count()` is retry pressure |
...
`refresh_failed`, `owner_not_found`, `clerk_error`, `timeout`), and the outcome
now follows the cause: `no_token` / `refresh_failed` / `owner_not_found` are
...
first two, "sign in to FGAC again, then re-delegate" with no link for a missing
