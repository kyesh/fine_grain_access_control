/**
 * Shared Google OAuth2 utilities for gmail-fgac skill.
 * Handles credential storage, token refresh, and authenticated API clients.
 * 
 * Supports both OAuth2 (Desktop app) and Service Account (FGAC.AI) credentials.
 */
...
/**
 * Get an authenticated OAuth2/Service Account client for the given account.
 * 
 * Service Account tokens (including FGAC.AI credentials) are detected by
 * the presence of `type: "service_account"` in the token JSON.
 * The Google SDK automatically handles routing based on universe_domain.
 */
