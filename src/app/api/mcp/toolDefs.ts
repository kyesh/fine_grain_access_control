...
export const TOOL_DEFS = {
  ...
  google_api_modify: {
    ...
    description: 'Perform a POST, PUT, or PATCH request against a Google API endpoint by path — the full write surface behind the typed convenience tools. Docs (https://developers.google.com/docs/api/reference/rest): v1/documents/{id}:batchUpdate supports tables, text styles, headings, images, and positional edits on documents with a Read & Write rule. Sheets (https://developers.google.com/sheets/api/reference/rest): every write endpoint on spreadsheets with a Read & Write rule, including v4/spreadsheets/{id}:batchUpdate for formatting, charts, and sheet management. Creating files is allowed and auto-granted to this connection: POST v1/documents, POST v4/spreadsheets, or POST slides/v1/presentations. Gmail (https://developers.google.com/gmail/api/reference/rest): mailbox writes are allowed by default — labels (create/update/apply), drafts (create/update), messages/{id}/modify (archive, mark read/unread), trash/untrash, messages/batchModify, insert/import. Sending is whitelisted: messages/send takes a base64url raw RFC 2822 body (HTML/MIME/threaded replies supported) and drafts/send resolves the draft\'s recipients server-side — every recipient must match the user\'s send whitelist. Gmail settings writes (aliases/sendAs, filters, forwarding) need Google scopes FGAC does not hold and are refused; permanent deletion (batchDelete) is never available. Comment writes (drive/v3/files/{id}/comments) require a Read & Write rule for the file — comments_add is the shortcut. Writes to other Drive-file APIs are forwarded under the per-file drive.file grant (files the user picked or this agent created). APIs outside the grant (People/Contacts, Calendar, Tasks, …) can NEVER work through FGAC and are refused — do not probe them. Batch endpoints are denied; DELETE is never available through FGAC. Denied calls return a one-click approval link for the user.',
    readOnly: false,
    destructive: true,
    openWorld: true,
