...
7. **Browser Automation**: In Claude Code, use the **built-in browser tools** (`mcp__Claude_Browser__*`) to analyze or test the UI — `preview_start`, `navigate`, `get_page_text`, `read_page`, `read_console_messages`, `read_network_requests`, `computer`, `resize_window` (viewport + light/dark emulation). NEVER write ad-hoc Node.js browser scripts. **Decision rule:** does this flow require the signed-in Google/Clerk session? No → built-in browser, always. Yes (anything behind `/dashboard`, Google OAuth consent, Clerk sign-in) → the Playwright CLI CDP path (Path B in `/browser-agent`), which attaches to the dedicated signed-in Chrome profile. Path B is the backup for that one case, not an alternative default — and within a mixed test, do the unauthenticated parts (public pages, console/network checks, theme/responsive) in the built-in browser even when other parts need CDP. See `/browser-agent` for both paths.
